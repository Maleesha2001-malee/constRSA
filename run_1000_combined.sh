...
echo ""
echo -e "${BOLD}==========================================${NC}"
echo -e "${BOLD} ConstRSA — 1000-Run Combined Experiment ${NC}"
echo -e "${BOLD}==========================================${NC}"
echo ""
...
def section(df, label):
    ...
    lines.append(f"    other  : {df['random_mean_ns'].mean():.0f} +/- {df['random_std_ns'].mean():.0f}")
    lines.append("")
...
out = []
out.append("")
out.append("  ConstRSA — 1000-Run Combined Timing Report")
